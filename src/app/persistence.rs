...
impl Persistence {
    pub fn new(state_dir: &Path) -> Self {
        Self { path: state_dir.join("state.db") }
    }
    ...
    fn load_inner(&self) -> rusqlite::Result<PersistedState> {
        ...
        state.sidebar_open = read_setting(&connection, "sidebar_open")
            .map(|value| value == "1")
            .unwrap_or(true);
        let mut shields = connection.prepare("SELECT key FROM settings WHERE key LIKE 'shield_off:%'")?;
    ...
    pub fn save(&self, urls: &[String], active_index: usize, sidebar_open: bool, disabled: &HashSet<String>) {
        if let Err(error) = self.save_inner(urls, active_index, sidebar_open, disabled) {
            eprintln!("ClearLane persistence warning: {error}");
        }
    }

    fn save_inner(&self, urls: &[String], active_index: usize, sidebar_open: bool, disabled: &HashSet<String>) -> rusqlite::Result<()> {
